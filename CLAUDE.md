...
The public **product page and customer portal** for the **Enigma2 Player** app (a
cross-platform Flutter client for Enigma2/OpenWebIF receivers, developed in the separate
`madoe21/enigma2_player` repo). Fully static Astro site on GitHub Pages, trilingual
(de default/unprefixed, en, es). Marketing pages plus a client-side portal
(register/login, buy via PayPal, manage licenses/devices/invoices) that talks to the
separate `license_server` REST API with **no secrets in the browser**.

- **Stack:** Astro 5 + TypeScript (`@astrojs/sitemap`; no client framework — vanilla-JS islands)
- **Entry point:** `src/pages/index.astro` (routes ×3 locales); config `astro.config.mjs`
- **Run locally:** `npm run dev` (build: `npm run build`, preview: `npm run preview`)
- **Test:** none yet — only `npm run check` (`astro check`). Introduce a test setup with the first substantial logic change (§3a).

> **Project aim (PROPOSED by `aiflow onboard` — please confirm or correct).**
> Give the Enigma2 Player app a professional public home: a fast, SEO-clean, trilingual
> static site that markets the app and lets customers buy and self-manage licenses
> entirely client-side against the license server. Target architecture: static-first
> Astro on GitHub Pages, JS islands only where interaction demands it, all privileged
> logic in the external license server, zero secrets shipped to the browser. Quality
> bar: every page prerendered, i18n/hreflang correct, portal degrades gracefully when
...
- Layering: `pages → layouts/components → i18n/lib`. `i18n` and `lib` import
  nothing internal. No layer skips.
- **Static-first:** every route prerendered; interactivity only as vanilla-JS
  inline `<script>` islands — no client framework, no runtime JS deps (ADR
  `docs/adr/0001-static-site-astro.md`).
- **Pages are thin wrappers ×3 locales** (`/`, `/en/`, `/es/`): import
  `layouts/Base.astro` + one shared `src/components/<X>Content.astro`, pass
  `lang`. New section ⇒ also register in `src/i18n/routes.ts` (`builtSections`)
  and add strings to `src/i18n/ui.ts`/`content.ts`.
- **i18n:** `de` default/unprefixed with fallback; never hardcode user-facing
  strings; never hand-build URLs/assets — use `href()`/`asset()` from
  `src/i18n/utils.ts` (base path `/enigma2_player` + locale aware).
- **License server access only via `src/lib/licenseApi.ts`** — the single API
  client; components never `fetch` the API directly. No secrets in the browser:
  only `PUBLIC_*` build-time env vars; session tokens in memory only.
- **Graceful degradation:** portal/buy/pricing must render a "not connected"
  note (not fail) when `PUBLIC_LICENSE_API_BASE`/`PUBLIC_APP_API_KEY` are unset.
- Errors: `ApiError(status, message)` from the API client; islands show
  translated error notes; never swallow.
- No persistence/database in this repo — all state lives in the external
  license server.
- Full architecture document: `docs/architecture/arc42.md` (arc42). Update it
